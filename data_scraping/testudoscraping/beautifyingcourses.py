...
def process(name):
    ...
    for course in courses_list:
        with open(r"/home/atajne/terpilot/data_scraping/raw/" + name + "/" + course + ".html", "r") as file:
        ...
        geneds_text = [re.sub('\s+', ' ', gened.text.replace('\n', '').strip()) for gened in geneds]
        geneds_text = list(set(geneds_text))
        if geneds_text != [] or geneds_text != ['']:
        ...
        '''for section in sections:
            ...
            locations = section.find('div', {'class': 'class-days-container'}).find_all('div', {'class': 'section-class-building-group push_one three columns'})
            times = section.find('div', {'class': 'class-days-container'}).find_all('div', {'class': 'section-day-time-group push_two five columns'})
            section_data['locations'] = ' | '.join([re.sub('\s+', ' ', location.find('span', {'class': 'class-building'}).text.replace('\n', '').strip()) for location in locations])
